...
export default function SuppliersModule() {
  ...
  const suppliers = (suppliersData?.suppliers || []).filter((s: Supplier) =>
  ...
  const getPOHTML = (purchase: Transaction) => {
    ...
    return `
      ...
      <body>
        ...
        ${purchase.notes ? `<p><strong>Catatan:</strong> ${purchase.notes}</p>` : ''}
        ...
        <script>
          async function handleShare() {
            const shareData = {
              ...
              text: 'PO ${purchase.invoiceNo} - ${supplier?.name || 'Supplier'} - Total: ${formatCurrency(purchase.total)}',
        ...
        <script>
          async function handleShare() {
            const shareData = {
              ...
              text: 'Request ${request.id.slice(0, 8).toUpperCase()} - ${supplier?.name || 'Supplier'} - Total: ${formatCurrency(request.amount)}',
              url: window.location.href
            };
                ...
                {purchaseRequests.map((r: FinanceRequest) => (
                  <div key={r.id} className="p-3 border rounded-lg space-y-1.5">
                    ...
                    <div className="flex items-center justify-end gap-1">
                      {r.status === 'processed' && r.goodsStatus !== 'received' && (
                        <Button 
                          variant="default" 
                          size="sm"
